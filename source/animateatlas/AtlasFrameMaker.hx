...
class AtlasFrameMaker extends FlxFramesCollection
{
	...
	*/
	...
	public static function construct(key:String, ?library:String, ?_excludeArray:Array<String> = null, ?noAntialiasing:Bool = false):FlxFramesCollection
	{
		...
		if (Paths.fileExists('images/$key/spritemap1.json', TEXT, false, library))
		{
		...
		var animationData:AnimationData = Json.parse(Paths.getTextFromFile('images/$key/Animation.json', false, library));
		var atlasData:AtlasData = Json.parse(Paths.getTextFromFile('images/$key/spritemap.json', false, library).replace("\uFEFF", ""));
