...
import FunkinLua;
import Stage;
#if hscript //this is sucks trust me :(
...
class FunkinStage {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "getRunningScripts", function(){
			...
			for (idx in 0...Stage.instance.luaArray.length)
				runningScripts.push(Stage.instance.luaArray[idx].scriptName);
		...
		Lua_helper.add_callback(lua, "callScript", function(?luaFile:String, ?funcName:String, ?args:Array<Dynamic>){
			...
			if(doPush)
			{
				for (luaInstance in Stage.instance.luaArray)
				{
		...
		Lua_helper.add_callback(lua, "addLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) { //would be dope asf.
			...
			if(doPush)
			{
				...
				Stage.instance.luaArray.push(new FunkinStage(cervix));
				return;
			}
					...
					for (luaInstance in Stage.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							//luaTrace('The script "' + cervix + '" is already running!');
								...
								Stage.instance.luaArray.remove(luaInstance);
							return;
						}
					}
				}
		...
		Lua_helper.add_callback(lua, "characterDance", function(character:String) {
			var char:Character; 
			if(character == '' || character == 'boyfriend' || character == 'bf')
				char = PlayState.instance.boyfriend; 
			else if((character == 'gf'||character == 'girlfriend') && PlayState.instance.gf != null)
		...
		Lua_helper.add_callback(lua, "makeLuaCharacter", function(tag:String, char:String, ?isPlayer:Bool = false, x:Float, y:Float) {
			luaTrace("makeLuaCharacter is deprecated! Use makeCharacter instead", false, true);
			tag = tag.replace('.', '');
			resetCharacterTag(tag);
			resetGroupTag(tag + 'Group');
			var leSprite:ModchartCharacter = new ModchartCharacter(0, 0, char, isPlayer);
			...
			Stage.instance.swagBacks.set(tag, leSprite);
			Stage.instance.swagBacks.set(tag + 'Group', leGroup);
		});
		Lua_helper.add_callback(lua, "makeLuaHealthIcon", function(tag:String, char:String, ?isPlayer:Bool = false) {
			tag = tag.replace('.', '');
			resetHealthIconTag(tag);
			var leSprite:ModchartHealthIcon = new ModchartHealthIcon(char, isPlayer);
			PlayState.instance.modchartHealthIcons.set(tag, leSprite);
			Stage.instance.swagBacks.set(tag, leSprite);
		});
		Lua_helper.add_callback(lua, "changeIcon", function(iconCharacter:String, char:String) {
			var icon:HealthIcon; 
			if(iconCharacter == '' || iconCharacter == 'boyfriend' || iconCharacter == 'bf' || iconCharacter == 'iconP1')
				icon = PlayState.instance.iconP1; 
			else if(iconCharacter == 'dad' || iconCharacter == 'iconP2')
				icon = PlayState.instance.iconP2; 
			else
				icon = getObjectDirectly(iconCharacter); 
		...
		Lua_helper.add_callback(lua, "addLuaHealthIcon", function(tag:String, front:Bool = false, ?layersName:String = 'boyfriend') {
		...
		Lua_helper.add_callback(lua, "changeLuaCharacter", function(tag:String, newCharacter:String) {
			if(PlayState.instance.modchartCharacters.exists(tag)) {
				...
				if(shit.curCharacter != newCharacter) {
					...
					shit.alpha = 0.00001;
					shit = PlayState.instance.charMap.get(newCharacter);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			if(pee.wasAdded) {
				Stage.instance.remove(pee, true);
				pee.wasAdded = false;
			}
		...
		Lua_helper.add_callback(lua, "setupStageObject", function(path:String) {
			var pathObject:String = 'objects/' + path + '.json';
			#if MODS_ALLOWED
			var path:String = Paths.modFolders(pathObject);
			if (!FileSystem.exists(path)) {
				path = Paths.getPreloadPath(pathObject);
			}
			if (!FileSystem.exists(path))
			#else
			var path:String = Paths.getPreloadPath(pathObject);
			#end

			
			if (FileSystem.exists(path)) {
				var jsonFormatted:Dynamic = File.getContent(path).trim();
		...
		Lua_helper.add_callback(lua, "characterPlayAnim", function(character:String, anim:String, ?forced:Bool = false, ?reverse:Bool = false, ?startFrame:Int = 0) {
			...
			if(char.animOffsets.exists(anim))
				char.playAnim(anim, forced, reverse, startFrame);
		});
	...
	function makeAnimatedLuaSprite(tag:String, image:String, x:Float, y:Float, ?spriteType:String = "sparrow")
	{
		tag = tag.replace('.', '');
		resetSpriteTag(tag);
		var leSprite:ModchartSprite = new ModchartSprite(x, y);
	...
	function addLuaSprite(tag:String, front:Bool = false, ?layersName:String = 'boyfriend')
	{
		if(PlayState.instance.modchartSprites.exists(tag)) {
			var shit:ModchartSprite = PlayState.instance.modchartSprites.get(tag);
			if(!shit.wasAdded) {
	...
	function scaleObject(obj:String, x:Float, y:Float, ?updateHitbox:Bool = true) {
		if(PlayState.instance.getLuaObject(obj)!=null) {
			var shit:FlxSprite = PlayState.instance.getLuaObject(obj);
			shit.scale.set(x, y);
			if(updateHitbox) shit.updateHitbox();
	...
	function getActorByName(id:String):Dynamic
	{
		...
		return  PlayState.instance.getLuaObject(id);
	}
}